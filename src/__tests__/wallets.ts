...
describe('Wallets and keys', () => {
  ...
  test('Generate valid JWKs', async () => {
    const walletA = await solid.wallets.generate();
    const walletB = await solid.wallets.generate();
    ...
    const addressA = await solid.wallets.jwkToAddress(walletA);
    const addressB = await solid.wallets.jwkToAddress(walletB);
  ...
  test('Get wallet info', async () => {
    ...
    const address = await solid.wallets.jwkToAddress(wallet);
    const balance = await solid.wallets.getBalance(address);
    const lastTx = await solid.wallets.getLastTransactionId(address);
    ...
    const balanceB = await solid.wallets.getBalance(liveAddress);
    const lastTxB = await solid.wallets.getLastTransactionId(liveAddress);
  ...
  test('Resolve JWK to Address', async () => {
    ...
    const address = await solid.wallets.jwkToAddress(jwk);
  ...
  test('Public key to address', async () => {
    const jwk = require('./fixtures/arweave-keyfile.json');
    const address = await solid.wallets.ownerToAddress(jwk.n);
