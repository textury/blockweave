...
const deepHash = async (data: DeepHashChunk): Promise<Uint8Array> => {
  ...
  if (Array.isArray(data)) {
    ...
    return await deepHashChunks(data, await Arsdk.crypto.hash(tag, 'SHA-384'));
  }

  tag = concatBuffers([stringToBuffer('blob'), stringToBuffer(data.byteLength.toString())]);

  const taggedHash = concatBuffers([await Arsdk.crypto.hash(tag, 'SHA-384'), await Arsdk.crypto.hash(data, 'SHA-384')]);

  return await Arsdk.crypto.hash(taggedHash, 'SHA-384');
};
...
export const deepHashChunks = async (chunks: DeepHashChunks, acc: Uint8Array): Promise<Uint8Array> => {
  ...
  const newAcc = await Arsdk.crypto.hash(hashPair, 'SHA-384');
