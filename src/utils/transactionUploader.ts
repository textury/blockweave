...
export class TransactionUploader {
  ...
  constructor(arsdk: Arsdk, transaction: Transaction | SerializedUploader | string, crypto: CryptoInterface) {
    this.arsdk = arsdk;
    this.crypto = crypto;
    ...
    if (transaction instanceof Transaction) {
      ...
      this.transaction = new Transaction(Object.assign({}, transaction, { data: new Uint8Array(0) }), this.arsdk);
    }
  }
  ...
  public async uploadChunk(): Promise<void> {
    ...
    const res = await this.arsdk.api.post(`chunk`, this.transaction.getChunk(this.chunkIndex, this.data)).catch((e) => {
  ...
  public static async fromSerialized(
    ...
    const upload = new TransactionUploader(arsdk, new Transaction(serialized.transaction, arsdk), crypto);
  ...
  private async postTransaction(): Promise<void> {
    ...
    if (uploadInBody) {
      ...
      try {
        res = await this.arsdk.api.post(`tx`, this.transaction);
      } catch (e) {
        console.error(e);
  ...
  async getUploader(
    ...
    if (upload instanceof Transaction) {
      uploader = new TransactionUploader(this.arsdk, upload, this.crypto);
    } else {
      ...
      if (typeof upload === 'string') {
        upload = await TransactionUploader.fromTransactionId(this.arsdk.api, upload);
      }

      // upload should be a serialized upload.
      uploader = await TransactionUploader.fromSerialized(this.arsdk, upload, data as Uint8Array, this.crypto);
    }
